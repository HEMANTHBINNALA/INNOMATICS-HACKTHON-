...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_profile'])) {
    ...
    $update_sql = "UPDATE users SET full_name='$full_name', email='$email', date_of_birth='$date_of_birth' WHERE id='$student_id'";
...
$stats_sql = "SELECT 
    ...
    AVG(CASE WHEN s.grade IS NOT NULL THEN 1 ELSE 0 END) * 100 as submission_grade_percentage
    FROM submissions s 
...
foreach ($grade_weight as $g => $points) {
    $count = $grades['grade_' . strtolower($g)] ?: 0;
    $grade_points += $count * $points;
    $graded_count += $count;
}

if ($graded_count > 0) {
    $avg_points = $grade_points / $graded_count;
    if ($avg_points >= 4.5) $overall_grade = "A";
    elseif ($avg_points >= 3.5) $overall_grade = "B";
    elseif ($avg_points >= 2.5) $overall_grade = "C";
    elseif ($avg_points >= 1.5) $overall_grade = "D";
    elseif ($avg_points >= 0.5) $overall_grade = "E";
...
<head>
    ...
    <title>My Profile - <?php echo htmlspecialchars($student['full_name']); ?></title>
...
<body>
    ...
    <div class="container" style="margin-top: 30px;">
        <a href="student_dashboard.php" class="back-button btn" style="background: rgba(255, 255, 255, 0.1); display: inline-flex; align-items: center; gap: 8px;">
        ...
        <div class="profile-header">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                <h1 style="margin: 0;"><i class="fas fa-user-circle" style="color: var(--accent-color); margin-right: 10px;"></i><?php echo htmlspecialchars($student['full_name']); ?>'s Profile</h1>
            ...
            <div id="view-mode" class="profile-info">
                ...
                <div class="info-item">
                    ...
                    <div class="info-value"><?php echo $student['date_of_birth'] ? date("M d, Y", strtotime($student['date_of_birth'])) : '(Not provided)'; ?></div>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <div class="info-value" style="text-transform: capitalize;"><?php echo htmlspecialchars($student['role']); ?></div>
                </div>
                <div class="info-item">
            ...
            <div id="edit-mode" style="display: none;">
                <form action="student_profile.php" method="POST" style="margin-top: 20px;">
                    <div class="profile-info">
                        <div class="info-item">
                            <label class="info-label"><i class="fas fa-user" style="margin-right: 8px;"></i>Full Name</label>
                            <input type="text" name="full_name" value="<?php echo htmlspecialchars($student['full_name']); ?>" style="width: 100%; padding: 8px; margin-top: 5px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,215,0,0.3); border-radius: 5px; color: white;" required>
                        </div>
                        <div class="info-item">
                            <label class="info-label"><i class="fas fa-envelope" style="margin-right: 8px;"></i>Email</label>
                            <input type="email" name="email" value="<?php echo htmlspecialchars($student['email'] ?: ''); ?>" style="width: 100%; padding: 8px; margin-top: 5px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,215,0,0.3); border-radius: 5px; color: white;" required>
                        </div>
                        <div class="info-item">
                            <label class="info-label"><i class="fas fa-birthday-cake" style="margin-right: 8px;"></i>Date of Birth</label>
                            <input type="date" name="date_of_birth" value="<?php echo htmlspecialchars($student['date_of_birth'] ?: ''); ?>" style="width: 100%; padding: 8px; margin-top: 5px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,215,0,0.3); border-radius: 5px; color: white;" required>
                        </div>
                    </div>
        ...
        <h2 style="margin-bottom: 20px; border-bottom: 2px solid var(--secondary-color); padding-bottom: 10px;">
            <i class="fas fa-chart-line" style="color: var(--accent-color); margin-right: 10px;"></i>Academic Performance
        </h2>

        <div class="overall-grade-section">
            <div class="overall-grade-text">YOUR OVERALL GRADE</div>
            <div class="overall-grade-large"><?php echo $overall_grade; ?></div>
        </div>
        ...
        <div class="stats-container">
            ...
            <div class="stat-card">
                <div class="stat-number"><?php echo $stats['assignments_submitted'] ?: 0; ?></div>
        ...
        <h2 style="margin-top: 40px; margin-bottom: 20px; border-bottom: 2px solid var(--secondary-color); padding-bottom: 10px;">
            <i class="fas fa-star" style="color: var(--accent-color); margin-right: 10px;"></i>Grade Distribution
        </h2>
